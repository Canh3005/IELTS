...
const handleSubmit = async () => {
  ...
  if (!testTitle.value || !task1Content.value || !task2Content.value) {
  ...
  try {
    const res = await fetch(
      `${import.meta.env.VITE_BASE_URL}/api/writing/create`,
      {
        method: "POST",
    ...
    if (res.ok) {
      alert("Tạo đề Writing thành công!");
      router.push("/writing"); // Chuyển hướng đến danh sách đề Writing
...
<template>
  <div class="bg-gray-100 min-h-screen flex flex-col items-center pt-10">
    ...
    >
      ...
      <EditorContent
        :editor="task1Content"
        class="border rounded min-h-[150px] p-2"
      />
